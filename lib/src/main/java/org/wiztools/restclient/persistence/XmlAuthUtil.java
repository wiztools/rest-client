...
class XmlAuthUtil {
        ...
        for (int i = 0; i < eChildren.size(); i++) {
            ...
            if (name.equals("token")) {
                ...
                throw new XMLException(
                    "Unknown element in oauth2-bearer auth: " + name
                );
            }
        }
    ...
    static void populateBasicDigestElement(
        Element eParent,
        BasicDigestAuth auth
    ) {
    ...
    static void populateUsernamePasswordElement(
        ...
        if (auth.getPassword() != null && auth.getPassword().length > 0) {
            ...
            ePassword.appendChild(
                Util.base64encode(new String(auth.getPassword()))
            );
    ...
    static Auth getAuth(Element eAuth) {
        ...
        for (int i = 0; i < eChildren.size(); i++) {
            ...
            if (name.equals("basic")) {
                ...
                throw new XMLException(
                    "Invalid auth element encountered: " + name
                );
            }
        }
        return null;
    ...
    static void populateBasicDigestAuth(
        BasicDigestAuthBaseBean bean,
        Element eAuth
    ) {
        Elements eChildren = eAuth.getChildElements();
        for (int i = 0; i < eChildren.size(); i++) {
            ...
            if (name.equals("host")) {
                ...
                throw new XMLException(
                    "Unknown element in basic/digest auth: " + name
                );
            }
        }
    }
