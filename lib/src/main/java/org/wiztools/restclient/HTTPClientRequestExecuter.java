...
public class HTTPClientRequestExecuter implements RequestExecuter {
    ...
    private static final Logger LOG = Logger.getLogger(
        HTTPClientRequestExecuter.class.getName()
    );
    ...
    public void execute(Request request, View... views) {
        ...
        final int urlPort = url.getPort() == -1
            ? url.getDefaultPort()
            : url.getPort();
        ...
        ProtocolVersion protocolVersion = httpVersion == HTTPVersion.HTTP_1_1
        ...
        IGlobalOptions options = ServiceLocator.getInstance(
            IGlobalOptions.class
        );
        ...
        if (request.getAuth() != null) {
            ...
            if (auth instanceof BasicAuth || auth instanceof DigestAuth) {
                ...
                String host = StringUtil.isEmpty(a.getHost())
                    ? urlHost
                    : a.getHost();
                String realm = StringUtil.isEmpty(a.getRealm())
                    ? null
                    : a.getRealm();
        ...
        try {
                ...
                if (bean != null) {
                    try {
                        if (bean instanceof ReqEntitySimple) {
                            AbstractHttpEntity e = HTTPClientUtil.getEntity(
                                (ReqEntitySimple) bean
                            );

                            reqBuilder.setEntity(e);
                        } else if (
                            bean instanceof ReqEntityMultipart multipart
                        ) {
                            ...
                            for (ReqEntityPart part : multipart.getBody()) {
                                ...
                                if (part instanceof ReqEntityStringPart) {
                                    ReqEntityStringPart p =
                                        (ReqEntityStringPart) part;
                                    String body = p.getPart();
                                    ContentType ct = p.getContentType();
                                    if (ct != null) {
                                        cb = new StringBody(
                                            body,
                                            HTTPClientUtil.getContentType(ct)
                                        );
                                    } else {
                                        cb = new StringBody(
                                            body,
                                            org.apache.hc.core5.http.ContentType.DEFAULT_TEXT
                                        );
                                    }
                                } else if (part instanceof ReqEntityFilePart) {
                                    ReqEntityFilePart p =
                                        (ReqEntityFilePart) part;
                                    File body = p.getPart();
                                    ContentType ct = p.getContentType();
                                    if (ct != null) {
                                        cb = new FileBody(
            ...
            if (tlsReq != null) {
                org.wiztools.restclient.bean.HostnameVerifier verifier =
                    tlsReq.getHostNameVerifier();
                ...
                SSLContext ctx = new SSLContextBuilder()
                    .loadKeyMaterial(
                        keyStore,
                        tlsReq.getKeyStore() != null
                            ? tlsReq.getKeyStore().getPassword()
                            : null
                    )
                ...
                final HttpClientConnectionManager cm =
                    PoolingHttpClientConnectionManagerBuilder.create()
                        .setTlsSocketStrategy(
                            new DefaultClientTlsStrategy(ctx, hcVerifier)
                        )
                        .build();
            ...
            httpClient.execute(
                req,
                httpContext,
                new HttpClientResponseHandler<HttpResponse>() {
                    @Override
                    public HttpResponse handleResponse(
                        ClassicHttpResponse http_res
                        ...
                        if (entity != null) {
                            if (request.isIgnoreResponseBody()) {
                                ...
                                try {
                                    byte[] responseBody =
                                        StreamUtil.inputStream2Bytes(is);
                        ...
                        try {
                            junit.framework.TestSuite suite =
                                TestUtil.getTestSuite(request, response);
            ...
            if (!interruptedShutdown) {
                ...
                try {
                    ...
                    LOG.log(
                        Level.WARNING,
                        "Exception when closing httpClient",
                        ex
                    );
                }
            } else {
