...
public class ReqAuthPanelImpl extends JPanel implements ReqAuthPanel {
    ...
    private final JComboBox<String> jcb_types = new JComboBox<String>(
        AuthHelper.getAll()
    );
    ...
    private void populateBasicDigestAuth(BasicDigestAuthBaseBean bean) {
        if (StringUtil.isNotEmpty(jtf_host.getText())) bean.setHost(
            jtf_host.getText()
        );
        if (StringUtil.isNotEmpty(jtf_realm.getText())) bean.setRealm(
            jtf_realm.getText()
        );
        if (StringUtil.isNotEmpty(jtf_username.getText())) bean.setUsername(
            jtf_username.getText()
        );
        if (jpf_password.getPassword().length > 0) bean.setPassword(
            jpf_password.getPassword()
        );
    ...
    public void setAuth(Auth auth) {
        if (auth instanceof BasicDigestAuth) {
            final String authType = auth instanceof BasicAuth
                ? AuthHelper.BASIC
                : AuthHelper.DIGEST;
            ...
            if (
                a.getPassword() != null && a.getPassword().length > 0
    ...
    protected void init() {
        ...
        // BASIC / DIGEST form:
        JPanel jp_form_label = new JPanel(
            new GridLayout(5, 1, RESTView.BORDER_WIDTH, RESTView.BORDER_WIDTH)
        );
        jp_form_label.add(new JLabel("<html>Host: </html>"));
        jp_form_label.add(new JLabel("<html>Realm: </html>"));
        jp_form_label.add(
            new JLabel("<html>Username: <font color=red>*</font></html>")
        );
        ...
        JPanel jp_form_input = new JPanel(
            new GridLayout(5, 1, RESTView.BORDER_WIDTH, RESTView.BORDER_WIDTH)
        );
        ...
        final JPanel jp_jsp_form = UIUtil.getFlowLayoutPanelLeftAligned(
            jp_form
        );

        // None Panel:
        final JPanel jp_none = UIUtil.getFlowLayoutPanelLeftAligned(
            new JPanel()
        );
        ...
        final JPanel jp_jsp_oauth2_bearer =
            UIUtil.getFlowLayoutPanelLeftAligned(jp_oauth2_bearer);

        // The Scrollpane:
        final JScrollPane jsp = new JScrollPane();
        jsp.setViewportView(jp_none);
        jcb_types.addActionListener(
            new ActionListener() {
                @Override
                public void actionPerformed(ActionEvent ae) {
                    final String selected =
                        (String) jcb_types.getSelectedItem();
